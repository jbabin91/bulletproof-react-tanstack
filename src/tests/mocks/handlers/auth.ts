...
import { AUTH_COOKIE, authenticate, hash, requireAuth } from '../utils';
...
export const authHandlers = [
    ...
    try {
      ...
      if (userObject.teamId) {
        ...
        const team = db.team.create({
          name: userObject.teamName ?? `${userObject.firstName}'s Team`,
        });
        persistDb('team');
        teamId = team.id;
        role = 'ADMIN';
  ...
  http.post(`${env.VITE_APP_API_URL}/auth/logout`, () => {
    // todo: remove once tests in Github Actions are fixed
  ...
  http.get(`${env.VITE_APP_API_URL}/auth/me`, ({ cookies }) => {
    try {
